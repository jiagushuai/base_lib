...
int toFormatPriPwd(EVP_PKEY *pEVPKey, char * base64, char *pwd)
{
	...
	BIO_get_mem_ptr(pBIO, &pBMem);
	baseLen = getEncodeLen(pBMem->length, (unsigned char *)pBMem->data);//�������ַ�������
	if (base64)
...
int toFormatPriPwdFile(EVP_PKEY *pEVPKey,char *pfxPwd)
{
	...
	iRV = i2d_PKCS8PrivateKey_bio(pBIO, pEVPKey, EVP_des_ede3_cbc(), NULL, 0, 0, pfxPwd);// ������i2d_PKCS8PrivateKey_bio_fp��ͬ
	if (iRV != 1)
	{
...
//		goto free;
//	}
//	/* 02���ֽ�X509�ṹ�õ�EVP_PKEY */
//	pEVPKey = X509_get_pubkey(x509);
//	if (NULL == pEVPKey) {
//		rv = SAR_PARAM_PARSE_ERR;
//		goto free;
//	}
//	/* 03��EVP_PKEYת����RSA��KEY */
//	*rsa = EVP_PKEY_get1_RSA(pEVPKey);
//	if (NULL == *rsa) {
